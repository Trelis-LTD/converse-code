...
def test_settings_register_completion_and_prompt_submission_hooks(tmp_path):
    path = write_settings(
        ...
        "http://127.0.0.1/hook/stop_failure",
    )

    settings = json.loads(path.read_text())
    hooks = settings["hooks"]
    assert set(hooks) == {"Stop", "StopFailure", "UserPromptSubmit", "PermissionRequest"}
    ...
    assert hooks["StopFailure"][0]["hooks"][0] == {
        "type": "http",
        "url": "http://127.0.0.1/hook/stop_failure",
        "timeout": 5,
    }

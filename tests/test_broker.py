...
async def test_start_frame_and_tool_roundtrip(mock_broker):
    ...
    calls, extra_json, audio_down = [], [], []

    async def on_tool_call(msg):
        calls.append(msg)
        await client.send_tool_progress(msg["id"], "checking things")
        await client.send_tool_partial_result(msg["id"], {"speak": "halfway"}, reply=True)
    ...
    # tool call down -> progress + partial + result up
    ...
    types = [(m["type"], m.get("note") or m.get("reply") or m.get("content", {}).get("speak")) for m in mock_broker.received]
    assert ("tool_progress", "checking things") in types
    assert ("tool_partial_result", True) in types

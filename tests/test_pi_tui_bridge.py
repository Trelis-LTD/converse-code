...
from converse_code.pi_tui import CommandReceipt, PiTUIBridge, PiTUIBridgeError
...
async def test_commands_are_id_correlated_and_events_are_forwarded():
    ...
    assert await pending == CommandReceipt(sent["id"], {
        "provider": "openai-codex", "model": "gpt-5.6-sol",
    })
...
async def test_malformed_acknowledgement_cannot_prove_command_acceptance():
    ...
    assert await pending == CommandReceipt(command_id, {})
